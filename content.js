...
function handlePaletteButtonClick(event) {
  ...
  const rawSelectedText = button.dataset.word;
  const style = button.dataset.style;
  const action = button.dataset.action;

  try {
    if (!rawSelectedText && action !== 'delete') {
      console.warn("Word Memory Assistant: No selected text found for palette action.");
      return; // Return early from try block, finally will still execute
    }

    // Normalize the text. The browser handles unescaping of attribute values.
    const selectedText = rawSelectedText.trim().toLowerCase().replace(/\s+/g, ' ');

    if (style) {
      if (isWordSaved(selectedText)) {
        if (getWordStyle(selectedText) !== style) {
          applyStyle(selectedText, style);
          showMessage(`Style for "${selectedText}" changed to ${style}.`, 'info');
        } else {
          showMessage(`"${selectedText}" is already style ${style}.`, 'info');
        }
      } else {
        addWordToList(selectedText, style);
      }
    } else if (action === 'delete') {
      if (isWordSaved(selectedText)) {
        removeWordFromList(selectedText);
      } else {
        showMessage(`"${selectedText}" is not in your list to delete.`, 'warning');
      }
    }
  } catch (e) {
    console.error("Word Memory Assistant: Error during palette button action:", e);
    // Optionally, show a generic error message to the user on the page:
    // showMessage("An error occurred. Please try again.", "error");
  } finally {
    hideStylePalette(); // Ensure palette is always hidden after an attempt.
  }
}
