...
document.addEventListener('click', function(event) {
  ...
  if (!clickedOnExtensionUI) {
    // If click was not on our UI elements, hide them.
    const selection = window.getSelection();
    if (!selection || selection.toString().trim() === '') {
        // If there's no active text selection after the click, hide everything.
        hideStylePalette(); // This also hides trigger and clears selection state.
    } else {
        // If there IS a new selection, mouseup -> handleTextSelection will show the trigger button.
        // So, we should still hide the palette if it was visible from a previous interaction.
        if (stylePalette && stylePalette.style.display === 'block') {
            hideStylePalette();
        }
        // Potentially, a click might have been to start a new selection.
        // We don't want to hide the trigger button if handleTextSelection will show it immediately.
        // The current logic in handleTextSelection (calling hideStylePalette first) handles this scenario.
    }
  }
...
document.addEventListener('mouseup', function(event) {
  // If the click is on the trigger button or palette, their own listeners should handle it.
    ...
    if (!triggerButton.hasEventListener) { // Simple flag to avoid multiple listeners
        triggerButton.addEventListener('click', function(event) {
            ...
            if (currentSelectedText) {
                ...
                // showAndPositionStylePalette(); // This function will be defined in Step 8
                console.log("Trigger button clicked, palette populated. Ready to show palette for:", currentSelectedText); // Placeholder
                hideTriggerButton(); // Hide trigger button once palette is to be shown
            }
        });
...
function hideTriggerButton() {
  ...
  // Also hide style palette if it's visible when trigger button is hidden due to new selection or invalid selection
  // if (stylePalette && stylePalette.style.display === 'block') {
  //   stylePalette.style.display = 'none';
  //   console.log("Style palette hidden because trigger button is being hidden.");
  // }
}
...
function populateStylePalette() {
  ...
  stylePalette.innerHTML = `
    <div class="action-row1">
      <button class="inline-style-btn" data-word="${safeSelectedText}" data-style="default" title="Default Style (Red)">2</button>
      <button class="inline-style-btn" data-word="${safeSelectedText}" data-style="green" title="Green Style">3</button>
      <button class="inline-style-btn" data-word="${safeSelectedText}" data-style="underline" title="Underline Style">4</button>
      <button class="inline-style-btn" data-word="${safeSelectedText}" data-action="delete" title="Remove">5</button>
      <button class="inline-style-btn" data-word="${safeSelectedText}" data-style="custom_a" title="Custom Style A">A</button>
      <button class="inline-style-btn" data-word="${safeSelectedText}" data-style="custom_b" title="Custom Style B">B</button>
    </div>
    <div class="action-row2">
      <button class="inline-style-btn" data-word="${safeSelectedText}" data-style="blue" title="Blue Style">6</button>
      <button class="inline-style-btn" data-word="${safeSelectedText}" data-style="yellow_bg" title="Yellow BG">7</button>
      <button class="inline-style-btn" data-word="${safeSelectedText}" data-style="bold" title="Bold Style">8</button>
      <button class="inline-style-btn" data-word="${safeSelectedText}" data-style="italic_underline" title="Italic Underline">9</button>
      <button class="inline-style-btn" data-word="${safeSelectedText}" data-style="custom_c" title="Custom Style C">C</button>
      <button class="inline-style-btn" data-word="${safeSelectedText}" data-style="custom_d" title="Custom Style D">D</button>
    </div>
  `;
